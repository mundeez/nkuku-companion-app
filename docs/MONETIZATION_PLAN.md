...
- **`journal_entries.organization_id` stays nullable for now** — 18 pre-existing test-generated entries in the dev DB have no organization, and backfilling them requires temporarily suspending the immutability `RULE` on that table (no UPDATE/DELETE). That's a financial-integrity control; suspending it — even briefly, even for test data — is a call for you to make explicitly, not something to do unilaterally. To close this out: either (a) confirm it's fine to purge those 18 test entries from the dev DB (they're all named things like "Test manual entry", "Immutability test cleanup"), or (b) approve a one-time supervised rule-suspend-and-backfill. Either way, all new journal entries created going forward already carry organizationId correctly (enforced at the application layer since Phase 1b) — this is a backfill-completeness gap, not a live-code gap.
