...
- **Deferred / known gaps** (not yet org-scoped, flagged rather than silently left): the legacy Milestone 1 planning tool (`production_cycles`/`batches`/`expansion-plan`/overhead-costs/mortality-records under `/api/v1/overhead`) and the deprecated `financial-engine` v0.8.0 endpoints (sunset 2027-01-01). These predate the broiler-management feature set, have no per-user isolation today either, and are lower-traffic/being sunset — recommend addressing in a follow-up pass before general self-serve signup launches (Phase 2), since any authenticated user can currently see all data in these specific legacy endpoints.
- `organizationId` columns are currently **nullable** (Phase 1c will make them `NOT NULL` once 100% of write paths are confirmed to set them, including the deferred modules above).
